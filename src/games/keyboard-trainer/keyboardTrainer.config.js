...
/**
 * Розкладка для вузьких екранів.
 *
 * Дванадцять клавіш у ряд на телефоні 320 px дають 24,17 px кожна — це впритул
 * до обов'язкового мінімуму WCAG 2.5.8 і, головне, впритул до товщини дитячого
 * пальця. Розширювати далі нікуди: ширина екрана скінченна.
 *
 * Тридцять дві літери лягають у чотири ряди по вісім рівно, без порожніх місць.
 * Порядок літер зберігається: розкладка лишається тією самою послідовністю,
 * просто перенесеною.
 *
 * Чесна ціна: на телефоні це вже не карта справжньої клавіатури, а поле для
 * пошуку літери. М'язова пам'ять під фізичну клавіатуру тут і так не працює —
 * її немає, — але тренажер на телефоні вчить трохи іншого, ніж на комп'ютері.
 */
const NARROW_ROW_LENGTH = 8

export const NARROW_KEYBOARD_ROWS = (() => {
  const flat = KEYBOARD_ROWS.flat()
  const rows = []
  for (let i = 0; i < flat.length; i += NARROW_ROW_LENGTH) {
    rows.push(flat.slice(i, i + NARROW_ROW_LENGTH))
  }
  return rows
})()
